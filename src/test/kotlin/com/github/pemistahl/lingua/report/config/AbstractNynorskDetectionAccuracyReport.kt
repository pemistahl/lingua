...
abstract class AbstractNynorskDetectionAccuracyReport(
    implementationToUse: LanguageDetectorImplementation
) : AbstractLanguageDetectionAccuracyReport(Language.NYNORSK, implementationToUse) {

    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/single-words/nn.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
    ...
    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/word-pairs/nn.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
    ...
    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/sentences/nn.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
