...
abstract class AbstractSomaliDetectionAccuracyReport(
    implementationToUse: LanguageDetectorImplementation
) : AbstractLanguageDetectionAccuracyReport(Language.SOMALI, implementationToUse) {

    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/single-words/so.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
    ...
    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/word-pairs/so.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
    ...
    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/sentences/so.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
