...
abstract class AbstractNorwegianDetectionAccuracyReport(
    implementationToUse: LanguageDetectorImplementation
) : AbstractLanguageDetectionAccuracyReport(Language.NORWEGIAN, implementationToUse) {

    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/single-words/no.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
    ...
    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/word-pairs/no.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
    ...
    @ParameterizedTest
    @CsvFileSource(resources = ["/language-testdata/sentences/no.txt"], delimiter = CSV_FILE_DELIMITER, encoding = CSV_FILE_ENCODING)
